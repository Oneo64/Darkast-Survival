...
public class StaticProjectile : MonoBehaviour
{
	...
	void Start() {
		...
		if (Camera.main != null && Vector3.Distance(Camera.main.transform.position, transform.position) > 30) {
	...
	private bool Check(Vector3 pos1, Vector3 pos2) {
		if (Physics.Linecast(pos1, pos2, out RaycastHit hit, LayerMask.GetMask(new string[] {"Default", "PlayerBody"}))) {
			...
			if (NetworkClient.localPlayer.isServer) {
				...
				float dmg2 = dmg / 100f;
				float newForce = speed * 20;

				if (hit.transform.name == "Neck") dmg *= 2;
				if ((hit.transform.name.Contains("Arm") || hit.transform.name.Contains("Leg") || hit.transform.name.Contains("Shin")) && !hit.transform.name.Contains("Armature")) {
					dmg = (int) Mathf.Round(dmg * 0.2f);
				}
			...
			blood = targ.GetComponentInParent<HasBlood>() != null;

			if (hit.transform.GetComponent<Rigidbody>()) hit.transform.GetComponent<Rigidbody>().AddForceAtPosition(transform.forward * force, hit.point);
	...
	private void CreateBulletHole(RaycastHit hit, RaycastHit hit2, bool blood) {
		...
		if (g.transform.Find("Sound")) {
			g.transform.Find("Sound").GetComponent<AudioSource>().clip = hitSounds[Random.Range(0, hitSounds.Length)];
			g.transform.Find("Sound").GetComponent<AudioSource>().Play();
		}
