	...
	public IEnumerator Attack() {
		if (!stalking || alert > Time.time) {
			...
			if (target != null && Vector3.Distance(transform.position, target.position) <= reach && Vector3.Dot(transform.forward, (target.position - transform.position).normalized) > 0.7f) {
				if (target.GetComponent<PlayerCore>()) target.GetComponent<PlayerCore>().RpcDamage(Random.Range(minDamage, maxDamage + 1), transform.forward * 500);
				if (target.GetComponent<Building>()) target.GetComponent<Building>().CmdDamage(Random.Range(minDamage, maxDamage + 1), transform.forward * 500);
			}
		} else {
			yield return null;
		}
